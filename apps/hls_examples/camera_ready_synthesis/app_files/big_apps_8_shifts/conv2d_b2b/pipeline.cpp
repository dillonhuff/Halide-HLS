...
public:
          ...
          win1(0, 2, 0, 2)
    {
      ...
      mul(x, y) = 0;
      mul(x, y) += cast<uint8_t>((hw_input(win.x + x, win.y + y) << kernel(win.x, win.y)));
      avg(x, y) =
        cast<uint8_t>(cast<uint8_t>(mul(x, y)) >> 4);

      mul1(x, y) = 0;
      mul1(x, y) += cast<uint8_t>((avg(win1.x + x, win1.y + y) << kernel1(win1.x, win1.y)));
      avg1(x, y) = 
        cast<uint8_t>(cast<uint8_t>(mul1(x, y)) >> 3);
      
      hw_output(x, y) = cast<uint8_t>(avg1(x, y));
